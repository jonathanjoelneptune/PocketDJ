...
// Menu 2.0 is now the normal entry point. The collapsed rail must be visible
// on a clean browser/incognito session, otherwise a new user has no way to open
// the menu after the legacy utility button was hidden.
let menu2HasOpened = localStorage.getItem("pocketdj-menu2-has-opened") !== "false";
if (localStorage.getItem("pocketdj-menu2-has-opened") !== "true") {
...
const ROOM_CAMERA_KEY = "pocketdj-room-camera-v1";
const DEFAULT_ROOM_CAMERA: RoomCameraSettings = { zoom: 1, x: 0, y: 0 };
let roomCamera = loadRoomCameraSettings();

function clampRoomCameraSettings(settings: Partial<RoomCameraSettings>): RoomCameraSettings {
  return {
    zoom: Math.max(0.75, Math.min(1.35, Number(settings.zoom ?? DEFAULT_ROOM_CAMERA.zoom))),
    x: Math.max(-600, Math.min(600, Number(settings.x ?? DEFAULT_ROOM_CAMERA.x))),
    y: Math.max(-600, Math.min(600, Number(settings.y ?? DEFAULT_ROOM_CAMERA.y))),
  };
}

function loadRoomCameraSettings(): RoomCameraSettings {
  try {
    const raw = localStorage.getItem(ROOM_CAMERA_KEY);
    if (!raw) return { ...DEFAULT_ROOM_CAMERA };
    return clampRoomCameraSettings({ ...DEFAULT_ROOM_CAMERA, ...JSON.parse(raw) });
  } catch {
    return { ...DEFAULT_ROOM_CAMERA };
  }
}

function saveRoomCameraSettings(): void {
  localStorage.setItem(ROOM_CAMERA_KEY, JSON.stringify(roomCamera));
}

function applyRoomCameraSettings(): void {
  const root = document.documentElement;
  root.style.setProperty("--room-camera-zoom", roomCamera.zoom.toFixed(3));
  root.style.setProperty("--room-camera-x", `${roomCamera.x.toFixed(1)}px`);
  root.style.setProperty("--room-camera-y", `${roomCamera.y.toFixed(1)}px`);

  const zoomInput = document.querySelector<HTMLInputElement>("#menu2RoomCameraZoom");
  const zoomValue = document.querySelector<HTMLElement>("#menu2RoomCameraZoomValue");
  const xInput = document.querySelector<HTMLInputElement>("#menu2RoomCameraX");
  const xValue = document.querySelector<HTMLElement>("#menu2RoomCameraXValue");
  const yInput = document.querySelector<HTMLInputElement>("#menu2RoomCameraY");
  const yValue = document.querySelector<HTMLElement>("#menu2RoomCameraYValue");
  if (zoomInput) zoomInput.value = roomCamera.zoom.toFixed(3);
  if (zoomValue) zoomValue.textContent = roomCamera.zoom.toFixed(3);
  if (xInput) xInput.value = roomCamera.x.toFixed(1);
  if (xValue) xValue.textContent = roomCamera.x.toFixed(1);
  if (yInput) yInput.value = roomCamera.y.toFixed(1);
...
function applyMenu2Settings(): void {
  ...
  panel.classList.toggle("menu2-side-left", menu2Side === "left");
  panel.classList.toggle("menu2-side-right", menu2Side === "right");
  ...
  document.documentElement.classList.toggle("menu2-side-left", menu2Side === "left");
  ...
  if (bubble) {
    bubble.classList.toggle("menu2-style-pocket", menu2StyleMode === "pocket");
    bubble.classList.toggle("menu2-style-spotify", menu2StyleMode === "spotify");
    bubble.classList.toggle("menu2-style-web", menu2StyleMode === "web");
...
function clearMenu2AutoCollapseTimer(): void {
  if (menu2AutoCollapseTimer) {
    window.clearTimeout(menu2AutoCollapseTimer);
...
function menu2Icon(name: "lyrics" | "play" | "pause" | "prev" | "next" | "volume" | "lock" | "unlock" | "compact" | "fullscreen" | "shuffle" | "repeat" | "queue" | "connect" | "openPanel" | "openPanelLeft" | "openPanelRight" | "broadcast" | "now" | "playlists" | "search" | "devices"): string {
  const paths: Record<typeof name, string> = {
    ...
    openPanelLeft: '<rect x="9.5" y="5" width="10" height="14" rx="2"/><path d="M13 9h3M13 12h3M13 15h2"/><path d="M4 12h7.5"/><path d="M8 8l4 4-4 4"/>',
    openPanelRight: '<rect x="4.5" y="5" width="10" height="14" rx="2"/><path d="M8 9h3M8 12h3M8 15h2"/><path d="M20 12H12.5"/><path d="M16 8l-4 4 4 4"/>',
...
async function loadMenu2Queue(): Promise<void> {
  ...
  try {
    ...
    container.innerHTML = menu2QueueTracks.length
      ? menu2QueueTracks.map((track, index) => menu2TrackRow(track, index).replace('<article class="menu2-row', `<article draggable="true" data-menu2-queue-index="${index}" class="menu2-row menu2-queue-draggable`)).join("")
...
async function performMenu2Search(source: "now" | "pane" = "now"): Promise<void> {
  ...
  const primaryType = document.querySelector<HTMLSelectElement>(source === "pane" ? "#menu2SearchPaneType" : "#menu2SearchType");
  const secondaryType = document.querySelector<HTMLSelectElement>(source === "pane" ? "#menu2SearchType" : "#menu2SearchPaneType");
  const container = document.querySelector<HTMLElement>("#menu2SearchResults");
  if (!primaryInput || !container) return;
  const query = primaryInput.value.trim();
  const type = (primaryType?.value || "all") as "track" | "artist" | "playlist" | "album" | "all";
  if (secondaryInput) secondaryInput.value = query;
  if (secondaryType && primaryType) secondaryType.value = primaryType.value;
  if (!query) {
    menu2SearchTracks = [];
    menu2SearchArtists = [];
...
function renderMenu2QueueFromState(): void {
  const container = document.querySelector<HTMLElement>("#menu2QueueResults");
  if (!container) return;
  container.innerHTML = menu2QueueTracks.length
    ? menu2QueueTracks.map((track, index) => menu2TrackRow(track, index).replace('<article class="menu2-row', `<article draggable="true" data-menu2-queue-index="${index}" class="menu2-row menu2-queue-draggable`)).join("")
...
function bindMenu2QueueDragEvents(): void {
  const container = document.querySelector<HTMLElement>("#menu2QueueResults");
  if (!container) return;
  container.addEventListener("dragstart", (event) => {
    const row = (event.target as HTMLElement).closest<HTMLElement>("[data-menu2-queue-index]");
    if (!row) return;
    menu2QueueDragIndex = Number(row.dataset.menu2QueueIndex || -1);
    event.dataTransfer?.setData("text/plain", String(menu2QueueDragIndex));
    row.classList.add("menu2-queue-dragging");
  });
  container.addEventListener("dragend", () => {
    menu2QueueDragIndex = null;
    container.querySelectorAll(".menu2-queue-dragging").forEach((row) => row.classList.remove("menu2-queue-dragging"));
  });
  container.addEventListener("dragover", (event) => {
    if (menu2QueueDragIndex === null) return;
    event.preventDefault();
  });
  container.addEventListener("drop", (event) => {
    const row = (event.target as HTMLElement).closest<HTMLElement>("[data-menu2-queue-index]");
    if (!row || menu2QueueDragIndex === null) return;
    event.preventDefault();
    const targetIndex = Number(row.dataset.menu2QueueIndex || -1);
    const sourceIndex = menu2QueueDragIndex;
    if (sourceIndex < 0 || targetIndex < 0 || sourceIndex === targetIndex) return;
    const next = [...menu2QueueTracks];
    const [moved] = next.splice(sourceIndex, 1);
    if (!moved) return;
    next.splice(targetIndex, 0, moved);
    menu2QueueTracks = next;
    renderMenu2QueueFromState();
    setMenu2Status("Queue order updated locally. Spotify only exposes add-to-queue from the browser API.", false);
  });
}
...
function bindMenu2Controls(): void {
  ...
  document.querySelector<HTMLButtonElement>("#menu2ReturnNow")?.addEventListener("click", () => setMenu2Tab("now"));
  bindMenu2QueueDragEvents();
  ...
  document.querySelector<HTMLButtonElement>("#menu2SearchButton")?.addEventListener("click", () => void performMenu2Search("now"));
  document.querySelector<HTMLInputElement>("#menu2SearchInput")?.addEventListener("keydown", (event) => {
    if (event.key === "Enter") void performMenu2Search("now");
  });
  document.querySelector<HTMLButtonElement>("#menu2SearchPaneButton")?.addEventListener("click", () => void performMenu2Search("pane"));
  document.querySelector<HTMLInputElement>("#menu2SearchPaneInput")?.addEventListener("keydown", (event) => {
    if (event.key === "Enter") void performMenu2Search("pane");
  });
  ...
  document.querySelector<HTMLSelectElement>("#menu2Side")?.addEventListener("change", (event) => {
    menu2Side = (event.target as HTMLSelectElement).value as typeof menu2Side;
    if (menu2Side !== "left" && menu2Side !== "right") menu2Side = "right";
  ...
  const bindRoomCameraSlider = (selector: string, key: keyof RoomCameraSettings) => {
    document.querySelector<HTMLInputElement>(selector)?.addEventListener("input", (event) => {
      roomCamera = clampRoomCameraSettings({ ...roomCamera, [key]: Number((event.target as HTMLInputElement).value) });
      saveRoomCameraSettings();
      applyRoomCameraSettings();
    });
  };
  bindRoomCameraSlider("#menu2RoomCameraZoom", "zoom");
  bindRoomCameraSlider("#menu2RoomCameraX", "x");
  bindRoomCameraSlider("#menu2RoomCameraY", "y");
  document.querySelector<HTMLButtonElement>("#menu2RoomCameraReset")?.addEventListener("click", () => {
    roomCamera = { ...DEFAULT_ROOM_CAMERA };
    saveRoomCameraSettings();
    applyRoomCameraSettings();
  });
...
function bindControls(): void {
  ...
  document.addEventListener("pointerdown", (event) => {
    handleMenu2OutsidePointer(event);
