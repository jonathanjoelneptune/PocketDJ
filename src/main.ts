...
function bindRoomUtilityControls(): void {
  ...
  const controls = [
    ...
    ["cameraX", "cameraXValue"],
    ["cameraY", "cameraYValue"],
    ["cameraZoom", "cameraZoomValue"]
  ] as const;
...
function applyRoomUtilitySettings(): void {
  ...
  root.style.setProperty("--room-camera-x", `${roomUtility.cameraX}%`);
  root.style.setProperty("--room-camera-y", `${roomUtility.cameraY}%`);
  root.style.setProperty("--room-camera-zoom", String(roomUtility.cameraZoom));
